...
#                         Data simulation                           #
#####################################################################
SimData0 <- function(lambda, theta, state, transSet){
  scv = c()        
  stl = list()     
  n <- length(theta)
  for (i in 1:n){
    ...
    while( TRUE ){
      ...
      if (s0 == 'A'){  s1 <- sample(state[1:2], 1, replace=T, p[1:2]) 
      }else if (s0 == 'B'){ s1 <- sample(state[1:3], 1, replace=T, p[3:5]) 
      }else if (s0 == 'C'){ s1 <- sample(state[c(1,2,4)], 1, replace=T, p[6:8])  
      }else if (s0 == 'D'){ s1 <- sample(state[2:5], 1, replace=T, p[9:12]) 
      }else if (s0 == 'E'){ s1 <- sample(state[c(3,5,6)], 1, replace=T, p[13:15]) 
      }else if (s0 == 'F'){ s1 <- sample(state[c(3,4,7)], 1, replace=T, p[16:18]) 
      }
...
CorMse <- function(theta,lambda,theHat,lamHat,csv){
  ...
  RmseThe <- sqrt(mseThe) 
  RmseLam <- sqrt(mean((lamHat-lambda)^2)) 
  cri <- c(corThe=corThe, corLam=corLam,abseThe=abseThe, abseLam=abseLam, 
           mseThe=mseThe, mseLam= mseLam, RmseThe=RmseThe,RmseLam=RmseLam) 
  cri_lam <- cbind(abseThe = abs(lamHat-lambda), RmseLam = (lamHat-lambda)^2)
...
plotMCMC <- function(lamMat, lambda, transSet){
  ...
  for (j in 1:u){
    ...
    plot(lamMat[j,,1] , type="l" , ylim = c(-5,5), ylab = nam, xlab = 'MCMC_Chain')
...
#                        Model fit : PPMC plot                      #                  
#####################################################################
ppmcPlot <- function(stateSet,transSet, theMat,lamMat){
  M <- dim(theMat)[[3]]
  L <- dim(theMat)[[2]]
  u <- length(transSet)
  ...
  for (m in 1:M){
    for (l in 9501:L){ #9501~10000
      ...
      # generate data
      dataList.l <- SimData0(lamMat[,l,m], theMat[,l,m], stateSet, transSet)
